var githubnaam = "https://larsm30.github.io/test_website";
var currentPath = window.location.pathname;
console.log(currentPath)


if (currentPath !== githubnaam + './clickedProduct.html'){
    loadCartRows();
}
...
function CheckInouts(){
    ...
    for (var i = 0 ; i < quantityInputs.length ; i++){
        ...
        if(currentPath !== githubnaam + "./Paypage.html"){
...
function updateCartTotal() {
    ...
    for (var i = 0; i < CartRows.length; i++) {
        ...
        var quantity = parseInt(QuantityElement.value); // Parse the quantity as an integer
        quantities.push(quantity); // Add the quantity to the array
        var nameElement = cartRow.getElementsByClassName("itemTitleCart")[0]
        namen.push(nameElement.innerText);
    ...
    console.log(quantities); // Log the list of quantities
}
...
function clickedProduct(product){
    ...
    window.location.href = githubnaam + "./clickedProduct.html";
}
...
function goToPayPage(){
    ...
    window.location.href = githubnaam + "./Paypage.html";
}
...
form.addEventListener('submit', (e) => {
    ...
    var emailadress = document.getElementsByClassName("email")[0];
    alert("uw email adres is: " + String(emailadress.value))
});
